...
<div class="header">
  ...
  <div class="header-text">
    ...
    <div class="meta">Skannet {{ timestamp }} &middot; {{ "%.1f"|format(result.file_size/1024) }} KB &middot; {{ result.text_length }} tegn lest{% if result.language %} &middot; {{ LANG_LABELS.get(result.language, result.language) }}{% endif %}</div>
...
<div class="stats">
  ...
  {% set count = result.findings|selectattr("severity","equalto",level)|list|length %}
  ...
  {% endfor %}
  <div class="stat">
    <div class="n">{{ result.findings|length }}</div>
    <div class="lbl">Totalt</div>
  </div>
</div>

{% if indexed_findings %}
<h2>Funn ({{ result.findings|length }})</h2>

{% if api_base %}
<div class="anon-section">
  <div class="anon-title">Anonymiser valgte funn</div>
  <div class="anon-bar">
...
<table>
  <thead>
    <tr>
      {% if api_base %}<th style="width:32px"></th>{% endif %}
      <th>Alvor</th><th>Kategori</th><th>Verdi</th><th>Kontekst</th>
      {% if api_base %}<th style="width:100px"></th>{% endif %}
    </tr>
  </thead>
  <tbody>
  {% for idx, f in indexed_findings %}
  <tr class="finding-row" data-idx="{{ idx }}">
    {% if api_base %}
    <td>{% if not f.category.startswith('⚠') %}<input type="checkbox" class="cb" value="{{ idx }}" checked>{% endif %}</td>
    {% endif %}
    ...
    {% if api_base %}
    <td>
      {% if (f.severity == 'gul' or f.severity == 'rød') and not f.category.startswith('⚠') %}
